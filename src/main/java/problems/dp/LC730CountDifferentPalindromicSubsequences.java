 ...
 *
 * https://www.leetfree.com/problems/count-different-palindromic-subsequences.html
 *
 * Solution:
...
public class LC730CountDifferentPalindromicSubsequences {
        ...
        for (int i = n-1; i >= 0; --i) {
            for (int j = i; j < n; ++j) {
                for (int x = 0; x < 4; ++x) {
                    char c = (char) ('a' + x);
                    if (j == i) {
                        if (S.charAt(i) == c) dp[x][i][j] = 1;
                        else dp[x][i][j] = 0;
                    }
                    else { // j > i
                        if (S.charAt(i) != c) dp[x][i][j] = dp[x][i+1][j];
                        else if (S.charAt(j) != c) dp[x][i][j] = dp[x][i][j-1];
                        else { // S[i] == S[j] == c
                            if (j == i+1) dp[x][i][j] = 2; // "aa" : {"a", "aa"}
                            else { // length is > 2
                                dp[x][i][j] = 2;
    ...
    public static void main(String[] args) {
        LC730CountDifferentPalindromicSubsequences countDifferentPalindromicSubsequences = new LC730CountDifferentPalindromicSubsequences();
