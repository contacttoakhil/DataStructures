 ...
 * }
 *
 * In count smaller number after self (LC315CountOfSmallerNumbersAfterSelf) we encountered the problem:
 ...
 *
 * The merge sort based solution counts the answer while doing the merge. During the merge stage, we have already sorted the left half [start, mid) and right half [mid, end). We then iterate through the left half with index i.
 ...
 */
public class LC367CountOfRangeSum {
    public int countRangeSum(int[] nums, int lower, int upper) {
        int n = nums.length;
        long[] sums = new long[n + 1];
        for (int i = 0; i < n; ++i)
    ...
    public static void main(String[] args) {
        LC367CountOfRangeSum countOfRangeSum = new LC367CountOfRangeSum();
